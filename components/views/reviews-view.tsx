...
import { Check, Copy, Eye, Filter, Link2, Loader2, MessageCircle, MoreVertical, Plus, Search, Send, Settings2, Share2, Sparkles, Star, Tag, Trash2, X, Zap } from 'lucide-react';
import { supabase, type Review, type BusinessProfile, type ReviewRequest, type ReviewTemplate } from '@/lib/supabase';
...
export function ReviewsView() {
  ...
  const sourceCounts = ['google', 'facebook', 'justdial', 'whatsapp', 'other'].map(src => ({ src, count: reviews.filter(r => r.source === src).length }));
  const totalForSource = sourceCounts.reduce((s, c) => s + c.count, 0) || 1;

  const save = async () => {
    if (!form.reviewer_name) {
      setToast('Reviewer name is required');
  ...
  const generateAiReply = async (review: Review) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error || 'AI reply generation failed');
      }
      const data = await res.json();
      setReviews(reviews.map(r => r.id === review.id ? { ...r, ai_reply: data.reply, ai_reply_at: new Date().toISOString() } : r));
      setToast('AI reply generated!');
      window.setTimeout(() => setToast(''), 2500);
    } catch (err: any) {
      setToast(err.message || 'AI reply failed. Please try again.');
      window.setTimeout(() => setToast(''), 3500);
    } finally {
  ...
  const saveReplyEdit = async (reviewId: string) => {
    await supabase.from('reviews').update({ ai_reply: replyDraft, ai_reply_at: new Date().toISOString() }).eq('id', reviewId);
    setReviews(reviews.map(r => r.id === reviewId ? { ...r, ai_reply: replyDraft } : r));
    setEditingReply(null);
    setReplyDraft('');
    setToast('Reply updated');
    window.setTimeout(() => setToast(''), 2500);
  };
  ...
  const sendRequest = async () => {
    ...
    await supabase.from('review_requests').insert({
      ...
      customer_phone: requestForm.customer_phone || null,
    ...
    const { data } = await supabase.from('review_requests').select('*').order('created_at', { ascending: false });
    setRequests(data || []);
  ...
  const saveTemplate = async () => {
    if (!templateForm.name || !templateForm.body) {
      setToast('Template name and body are required');
      window.setTimeout(() => setToast(''), 2500);
      return;
    }
    if (editingTemplate) {
      await supabase.from('review_templates').update({ ...templateForm, updated_at: new Date().toISOString() }).eq('id', editingTemplate.id);
      setToast('Template updated');
    } else {
      await supabase.from('review_templates').insert({ ...templateForm, company_id: companyId });
      setToast('Template created');
    }
    setShowTemplateForm(false);
    setEditingTemplate(null);
    setTemplateForm({ name: '', channel: 'whatsapp', subject: '', body: '' });
    const { data } = await supabase.from('review_templates').select('*').order('created_at', { ascending: false });
    setTemplates(data || []);
    window.setTimeout(() => setToast(''), 2500);
  };
  ...
  const removeTemplate = async (id: string) => {
    await supabase.from('review_templates').delete().eq('id', id);
  ...
  return (
    <>
      <div className="page-header">
        <div>
          <h2 className="page-title">AI Review Management</h2>
          <p className="page-subtitle">Collect, monitor, and respond to customer reviews across all platforms</p>
        </div>
          ...
          {tab === 'reviews' && (
            <>
              <button className="ghost-btn" onClick={() => { setReviewLinkCopied(true); navigator.clipboard.writeText(reviewLink); window.setTimeout(() => setReviewLinkCopied(false), 2000); }}>
                {reviewLinkCopied ? <><Check size={15} /> Link Copied!</> : <><Link2 size={15} /> Copy Review Link</>}
              </button>
      ...
      {tab === 'reviews' && (
        <>
          <div className="rep-stats-row">
            ...
            <div className="rep-stat-card"><div className="rep-stat-icon attention"><Zap size={18} /></div><div><strong>{needAttention} ({totalReviews > 0 ? Math.round((needAttention / totalReviews) * 100) : 0}%)</strong><span>Need Attention</span></div></div>
            <div className="rep-stat-card"><div className="rep-stat-icon response"><MessageCircle size={18} /></div><div><strong>{responseRate}%</strong><span>Response Rate</span></div></div>
          </div>
          ...
          {(loading || companyLoading) ? (
            ...
            <div className="empty-state">
              ...
              <p>{search || sourceFilter !== 'all' || statusFilter !== 'all' ? 'Try different filters.' : 'Add your first review or send a review request.'}</p>
            </div>
          ) : (
